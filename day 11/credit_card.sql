...
-- left join : return all rows from left and matching rows from right, it returns null if no match in the right
select card_holder, transactions.t_date, t_amount
from creditcard left join
transactions on card_no = transactions.t_by;

-- right join : returns all rows from right, and matching rows from the left
...
-- instead union of left and right can be used
select card_holder, transactions.t_id,
merchant.merchant_name, transactions.t_amount from
creditcard left join transactions
on card_no = transactions.t_by left join merchant on
transactions.t_to = merchant.merchant_id
union
select card_holder, transactions.t_id,
merchant.merchant_name, transactions.t_amount from
creditcard right join transactions on
card_no = transactions.t_by right join merchant
on transactions.t_to = merchant.merchant_id;

-- insert into transactions values(
-- aggregate functions : sum, avg, max
select creditcard.card_holder, sum(t_amount) as spent
from transactions join creditcard on t_by = creditcard.card_no
group by creditcard.card_holder;

-- average
select merchant.merchant_name, avg(t_amount) as AverageIncome
from transactions join merchant on t_to = merchant.merchant_id
...
grant create on placement_training to 'head';
